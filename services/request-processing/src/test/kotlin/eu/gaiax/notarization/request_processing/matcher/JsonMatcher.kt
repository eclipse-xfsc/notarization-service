...
class WithJsonField<T: JsonNode>(
    ...
    companion object {
        ...
        fun <T : JsonNode> withoutJsonField(property: String): WithoutJsonField<T> {
            return WithoutJsonField(property)
        }
    }
}

class WithoutJsonField<T: JsonNode>(
    private val fieldName: String
) : TypeSafeDiagnosingMatcher<T>(JsonNode::class.java) {
    ...
    override fun matchesSafely(item: T, mismatchDescription: Description): Boolean {
        val jsonNode = item[fieldName]
        if (jsonNode != null) {
            mismatchDescription.appendText("a field ").appendValue(fieldName).appendText(" found")
            return false
        }
        return true
    }
