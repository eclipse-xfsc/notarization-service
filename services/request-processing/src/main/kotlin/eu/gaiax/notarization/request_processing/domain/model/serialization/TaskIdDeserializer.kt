 ...
 * @author Florian Otto
 */
class TaskIdDeserializer : JsonDeserializer<TaskId>() {
    @Throws(IOException::class, JacksonException::class)
    override fun deserialize(jp: JsonParser, dc: DeserializationContext): TaskId {
        val text = jp.text
        return TaskId(UUID.fromString(text))
    }
}
