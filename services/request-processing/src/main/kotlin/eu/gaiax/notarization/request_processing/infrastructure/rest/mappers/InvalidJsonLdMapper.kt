 ...
 */
@Provider
class InvalidJsonLdMapper : ExceptionMapper<InvalidJsonLdException?> {
    @APIResponse(
        responseCode = "400",
        description = "Invalid JSON-LD",
        content = [Content(
            mediaType = MediaType.APPLICATION_JSON,
            schema = Schema(implementation = InvalidJsonLdDetails::class)
        )]
    )
    @Produces(
        MediaType.APPLICATION_JSON
    )
    override fun toResponse(x: InvalidJsonLdException?): Response {
    ...
    fun mapException(x: InvalidJsonLdException?): RestResponse<ProblemDetails> {
        logger.debug("Handled invalid json-ld exception", x)
        return RestResponse.status(Response.Status.BAD_REQUEST, asDetails())
    }

    companion object {
        fun asDetails(): InvalidJsonLdDetails {
            return InvalidJsonLdDetails(
                ProblemDetails.Companion.ABOUT_BLANK, "Invalid JSON-LD", "Provided JSON-LD is not valid", null
            )
        }
    }
}
